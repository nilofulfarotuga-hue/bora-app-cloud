-- BORA — Sessão 3 (saco mercado): RPC finalize_storeshopping_purchase
--
-- Mudanças:
--   1. Cap bag_count: 0..20 → 0..5 (política €0.50 max)
--   2. Branch CASH com extra > 0:
--      UPDATE orders SET cash_total_due = COALESCE(cash_total_due, 0) + extra
--      → estafeta cobra "em mão" via banner
--   3. Branch card/mbway mantém comportamento actual (extraRequired)
--      Charge automático ficará para Sessão 3B (charge-extra off_session)
--
-- TUDO MAIS mantém-se idêntico à versão 20260502010000:
--   - SECURITY DEFINER + search_path=public
--   - bypass enforce_financial_immutability via app.financial_bypass GUC
--   - audit log em admin_audit_log
--   - cálculo bought/unavailable/added/bag/delivery/service fees
--   - refund/extra dispatch idêntico
...
BEGIN
  ...
  ELSE
    IF p_bag_count IS NOT NULL THEN
      -- Cap 0..5 (política €0.50 max — Sessão 3 / 2026-05-04)
  ...
  ELSIF v_final_total_cents > v_orig_total_cents THEN
    IF v_is_card THEN
      -- Sessão 3: marca extraRequired. Sessão 3B implementa charge automático
      -- via pending_charges + Edge Function charge-extra off_session.
      v_extra_charge_cents := v_final_total_cents - v_orig_total_cents;
      v_new_payment_status := 'extraRequired';
    ELSE
      -- CASH: estafeta cobra em mão. Acumula em cash_total_due.
  ...
  UPDATE public.orders SET
    ...
    extra_charge_amount   = CASE WHEN v_extra_charge_cents > 0
                                 ...
                                 ELSE NULL END,
    -- Sessão 3: cash_total_due acumula extra para cash. Card/mbway não toca.
    cash_total_due        = CASE
                              WHEN v_cash_extra_cents > 0
                                THEN COALESCE(cash_total_due, v_final_total_cents::numeric / 100.0)
                              ELSE cash_total_due
                            END,
