-- APLICADA EM PRODUÇÃO pela Claude.ai (MCP) — version 20260921064817 · contas_claras_b9_recibo_acentos_ptpt_2026_09_21.
...
-- A migration original foi aplicada por SUBSTITUIÇÃO DE TEXTO (bloco DO) na definição viva de weekly_closeout_compile;
...
--   -- CONTAS CLARAS B9 (21/09/2026) — acentos na linha que o motorista le.
--   -- O recibo vai em PT-PT e ja leva <meta charset="utf-8">, por isso os acentos
--   -- saem bem. A etiqueta do dinheiro em mao tinha ficado sem eles.
--   -- Feito por substituicao do texto na definicao viva da funcao, para nao haver
--   -- risco de erro a recopiar 200 linhas de SQL de dinheiro a mao.
--   -- NOTA PARA O REPO: o ficheiro da weekly_closeout_compile tem de ficar com a
--   -- etiqueta acentuada 'Dinheiro que recebeu em mão (devolve à Bora)'.
--   DO $do$
--   DECLARE
--     v_def text;
--   BEGIN
...
--     WHERE n.nspname = 'public' AND p.proname = 'weekly_closeout_compile';
--   
--     IF position('Dinheiro que recebeu em mao (devolve a Bora)' in v_def) = 0 THEN
--       RAISE EXCEPTION 'etiqueta nao encontrada — nada alterado';
--     END IF;
--   
--     v_def := replace(v_def,
--       'Dinheiro que recebeu em mao (devolve a Bora)',
--       'Dinheiro que recebeu em mão (devolve à Bora)');
--   
--     EXECUTE v_def;
--   END $do$;
