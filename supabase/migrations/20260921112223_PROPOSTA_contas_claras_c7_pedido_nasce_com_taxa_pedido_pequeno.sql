-- 2026-09-21 — CONTAS CLARAS · Bloco C7 · PROPOSTA (zona vermelha: a Trava do PC recusa DDL que nomeie a
-- função que cria o pedido; aplica a Claude.ai/o Danilo). AUTORIZADA pelo Danilo a 21/09 ("VAI": cobrar o que
-- o carrinho já mostra e já está configurado; não muda o valor da taxa (139) nem o mínimo (1500)).
--
-- O QUE MEXE: a função que cria o pedido passa a somar a taxa de pedido pequeno (valor GLOBAL, como o quote)
-- ao total do cliente logo a seguir ao pricing — por isso o tecto da carteira, o charge_total, o
-- payment_buffer_total, o estado de pagamento, o price gravado e o JSON devolvido ficam TODOS com ela — e
-- declara-a na coluna small_order_fee do INSERT. O gatilho orders_aa_small_order_fee (corrigido na migration
-- irmã, já aplicada) só acrescenta a diferença da loja. Sem esta função corrigida o gatilho já põe a taxa
-- em price/final_total/payment_buffer_total; ficam de fora só o tecto da carteira, o charge_total/estado
-- de pagamento e o JSON devolvido — é isso que esta proposta fecha.
--
-- 3 edições sobre o corpo VIVO (pg_get_functiondef a 21/09 10:50 UTC, 29078 chars), marcadas 'CONTAS CLARAS C7':
--   1. DECLARE: v_small_order_fee NUMERIC := 0;
--   2. depois do pricing (e do ramo takeaway): v_small_order_fee := small_order_fee_calc(service_type, subtotal_server, NULL);
--      v_customer_total := ROUND(v_customer_total + v_small_order_fee, 2)  — a mesma linha do quote_order_pricing;
--   3. INSERT ... small_order_fee = v_small_order_fee; RETURN ... 'small_order_fee', v_small_order_fee.
-- Errand e takeaway: small_order_fee_calc devolve 0 (só restaurant/storeShopping) — nada muda.
--
-- PROVA A FAZER DEPOIS DE APLICAR (rollback, DO + RAISE, JWT do cliente demo): Goola Bowl 9,22 (parceiro) →
-- price = quote.customer_total = 13,87, small_order_fee 1,39, payment_buffer_total 13,87; Wells 1×3,68 →
-- price 9,21 = quote, buffer 10,59 = quote; Wells acima do mínimo → taxa 0, nada muda; carteira a cobrir
-- tudo → wallet_applied pode chegar ao total COM taxa.
...
DECLARE
  ...
  v_bag_fee            NUMERIC := 0;
  -- CONTAS CLARAS C7 (21/09/2026): taxa de pedido pequeno, a MESMA que o quote mostra ao cliente.
...
BEGIN
  ...
  --╔══BEGIN_SEC_HARDENING══╗
  -- SEC-1 (Tarefa 1): is_partner_store é spoofable no input do cliente. Server-side:
  --   * carry/send → forçar FALSE (não há loja)
  --   * restaurant/storeShopping → lookup em restaurants.is_partner pelo restaurant_id
  -- Pre-lookup: garante que v_restaurant_id está populado mesmo quando o cliente
  -- mandou is_partner_store=FALSE (anti-spoof — para que o lookup abaixo funcione).
  IF v_restaurant_id IS NULL AND v_vendor_name IS NOT NULL THEN
    SELECT id INTO v_restaurant_id FROM public.restaurants
      WHERE name = v_vendor_name LIMIT 1;
  END IF;
  IF v_service_type IN ('carryGroceries','sendPackage') THEN
    v_is_partner_store := FALSE;
  ELSIF v_service_type IN ('restaurant','storeShopping') AND v_restaurant_id IS NOT NULL THEN
  ...
  END IF;
  -- SEC-2 (Tarefa 1): distance_km validada vs haversine. Tolerância 20%.
  -- Pedidos legítimos (distância real ≈ Google Routes) passam folgados;
  -- spoofs (distance_km << haversine) são bloqueados.
  DECLARE
    v_min_km_co NUMERIC;
    v_r_lat_co DOUBLE PRECISION;
    v_r_lng_co DOUBLE PRECISION;
  BEGIN
    IF v_service_type IN ('restaurant','storeShopping') AND v_restaurant_id IS NOT NULL THEN
      SELECT lat, lng INTO v_r_lat_co, v_r_lng_co FROM public.restaurants WHERE id = v_restaurant_id;
      IF v_r_lat_co IS NOT NULL AND v_r_lng_co IS NOT NULL AND v_dropoff_lat IS NOT NULL AND v_dropoff_lng IS NOT NULL THEN
        v_min_km_co := public._haversine_km(v_r_lat_co::numeric, v_r_lng_co::numeric, v_dropoff_lat::numeric, v_dropoff_lng::numeric);
      END IF;
    ELSIF v_service_type IN ('carryGroceries','sendPackage') THEN
      IF v_pickup_lat IS NOT NULL AND v_pickup_lng IS NOT NULL AND v_dropoff_lat IS NOT NULL AND v_dropoff_lng IS NOT NULL THEN
        v_min_km_co := public._haversine_km(v_pickup_lat::numeric, v_pickup_lng::numeric, v_dropoff_lat::numeric, v_dropoff_lng::numeric);
      END IF;
    END IF;
    IF v_min_km_co IS NOT NULL AND v_distance_km < (v_min_km_co * 0.8) THEN
      RAISE EXCEPTION 'DISTANCE_MISMATCH: sent=%, min_haversine=%', v_distance_km, v_min_km_co USING ERRCODE='23514';
    END IF;
  END;
  --╚══END_SEC_HARDENING══╝
  ...
  IF v_service_type = 'errand' THEN
    ...
    BEGIN
      ...
      IF v_e_min_km IS NOT NULL AND v_distance_km < (v_e_min_km * 0.8) THEN
        RAISE EXCEPTION 'ERRAND_DISTANCE_TOO_LOW: sent=%, min_haversine=%', v_distance_km, v_e_min_km USING ERRCODE='23514';
      END IF;
      ...
      v_subtotal_server := v_subtotal_server + (
        CASE WHEN NOT v_is_partner_store AND NOT v_is_takeaway THEN
          ROUND((COALESCE(
            ...
            (v_line->>'unit_price')::NUMERIC, 0) * 1.15)::numeric, 2)
        ELSE
          COALESCE(
            (SELECT p.price FROM products p WHERE p.id = (v_line->>'product_id') LIMIT 1),
          ...
          IF v_line_extras > 0 THEN
            v_subtotal_server := v_subtotal_server
              + (CASE WHEN NOT v_is_partner_store AND NOT v_is_takeaway
                      THEN ROUND((v_line_extras * 1.15)::numeric, 2)
  ...
  -- CONTAS CLARAS C7 (21/09/2026): a taxa de pedido pequeno entra no total do pedido.
  -- Ate aqui so quote_order_pricing a somava (o ecra do carrinho); o pedido nascia SEM ela e
  -- o cliente pagava menos do que viu (5 pedidos, 6,95 EUR; nos parceiros nunca era cobrada).
  -- Valor GLOBAL, exactamente como no quote: o que o cliente ve e o que fica em price.
  -- O gatilho orders_aa_small_order_fee recebe em small_order_fee o que aqui se somou e so
  -- acrescenta a diferenca da loja (override), quando a houver.
  v_small_order_fee := COALESCE(public.small_order_fee_calc(v_service_type, v_subtotal_server, NULL), 0);
  v_customer_total  := ROUND((v_customer_total + v_small_order_fee)::numeric, 2);
