-- ============================================================================
-- BORA — Sessão 3B-NOVA — finalize_storeshopping_purchase usa wallet (B4)
-- ============================================================================
-- Quando há extra a cobrar em pedidos card/mbway:
--   wallet_negative_enabled = true  → debita wallet (pode ficar negativa)
--   wallet_negative_enabled = false → fallback Sessão 3 (extraRequired)
-- Cash mantém comportamento Sessão 3 (cash_total_due).
-- Refund branch (refundPending) intacto.
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_final_total_cents > v_orig_total_cents THEN
    IF v_is_card THEN
      ...
      -- Sessão 3B-NOVA: debita wallet (pode ficar negativa)
      IF v_wallet_neg_enabled THEN
        ...
        BEGIN
          ...
          v_extra_charge_cents := 0;             -- já cobrado via wallet
          v_new_payment_status := v_order.payment_status;  -- mantém 'paid'
        EXCEPTION WHEN OTHERS THEN
          -- Falha (ex: hard_floor_exceeded) → fallback extraRequired
        ...
        END;
      ELSE
        -- Kill-switch OFF → comportamento Sessão 3
        v_new_payment_status := 'extraRequired';
      END IF;
    ELSE
      -- CASH (Sessão 3 mantém)
  ...
  -- Warning original sobre cap pct (ainda útil para auditoria)
