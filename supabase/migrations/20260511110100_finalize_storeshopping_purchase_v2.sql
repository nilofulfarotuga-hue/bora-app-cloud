-- ═══════════════════════════════════════════════════════════════════════════
-- BLOCO 3.3 — finalize_storeshopping_purchase_v2 (paralelo a v1, NÃO TOCA v1)
--
-- Estafeta chama no fim da compra na loja com:
--   - p_order_id
--   - p_driver_typed_total_cents (valor pago no caixa)
--   - p_receipt_photo_url (Supabase Storage path)
--   - p_items (array)
--
-- Side-effects:
--   1. orders.purchase_flow_version = 2
--   2. INSERT order_receipts_v2 (reimbursement_status = cash_settled OR pending_admin)
--   3. INSERT order_purchase_items_v2 (loop sobre p_items)
--   4. Crédito wallet cliente para items unavailable (refund_credit_free)
--   5. POST notify-admin-reimbursement (apenas Stripe/MBWay)
--   6. POST ocr-receipt (sempre — Decisão H shadow)
--   7. POST notify-purchase-finalized (Decisão I sempre)
--   8. orders.status = 'onTheWay'
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT id, user_id, assigned_driver_id, service_type, is_partner_store, payment_method
    INTO v_order
    FROM public.orders
   WHERE id = p_order_id;
  ...
  IF v_url IS NOT NULL AND v_url <> '' AND v_key IS NOT NULL AND v_key <> '' THEN
    ...
    END;
  ELSE
    RAISE NOTICE '5G v2: pg_net settings MISSING — pushes/OCR skipped for order %', p_order_id;
  END IF;
...
REVOKE ALL ON FUNCTION public.finalize_storeshopping_purchase_v2(text, integer, text, jsonb) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.finalize_storeshopping_purchase_v2(text, integer, text, jsonb) TO authenticated;

COMMENT ON FUNCTION public.finalize_storeshopping_purchase_v2(text, integer, text, jsonb) IS
  '5G — v2 paralelo. Driver-only, non-partner storeShopping. Dispara notify-admin-reimbursement (Stripe/MBWay) + ocr-receipt + notify-purchase-finalized. Avança status onTheWay.';
