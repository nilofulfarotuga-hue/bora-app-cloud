-- ============================================================
-- Batch D — partner commission columns + create_order update
-- 2026-04-25
--
-- Adds 3 columns to orders to track the split commission:
--   partner_commission_visible  = 10% (partner pays at settlement)
--   partner_markup_hidden       = 5%  (embedded in product prices)
--   partner_service_fee_client  = 5%  (charged to client at checkout)
-- ============================================================

ALTER TABLE orders
  ADD COLUMN IF NOT EXISTS partner_commission_visible NUMERIC DEFAULT 0,
  ADD COLUMN IF NOT EXISTS partner_markup_hidden      NUMERIC DEFAULT 0,
  ADD COLUMN IF NOT EXISTS partner_service_fee_client NUMERIC DEFAULT 0;

-- Update create_order: use RECORD type for pricing_calculate result,
-- populate new partner columns, expose partner_markup_hidden in RPC response.
...
BEGIN
  ...
  INSERT INTO orders (
    ...
    CASE WHEN p_input->>'bag_fee' IS NOT NULL THEN (p_input->>'bag_fee')::NUMERIC ELSE 0 END,
  ...
  RETURN jsonb_build_object(
    ...
    'bag_fee',                   CASE WHEN p_input->>'bag_fee' IS NOT NULL THEN (p_input->>'bag_fee')::NUMERIC ELSE 0 END,
