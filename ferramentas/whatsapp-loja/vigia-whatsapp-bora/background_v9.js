/* Service worker da Vigia Bora v9: faz as chamadas ao cerebro local (127.0.0.1:8790).
 * Corre com host_permissions -> sem CSP da pagina e sem CORS. O content.js pede por mensagem.
 * (Ficheiro renomeado a cada versao: o Chrome nao recarrega o worker de uma extensao descompactada
 *  so por o ficheiro mudar -- lição de 02/09.) */
...
chrome.runtime.onMessage.addListener((req, _sender, sendResponse) => {
  (async () => {
    try {
      ...
      if (req.tipo === 'comandos') return sendResponse(await get('/comandos', 6000));
      ...
      if (req.tipo === 'comando_resultado') return sendResponse(await post('/comando_resultado', req.dados, 8000));
