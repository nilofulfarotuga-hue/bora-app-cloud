-- APLICADA EM PRODUÇÃO pela Claude.ai (MCP) — version 20260921064611 · contas_claras_b9_recibo_mostra_corridas_2026_09_21.
...
-- CONTAS CLARAS B9 (21/09/2026) — o recibo semanal do estafeta/motorista.
--
-- O QUE ESTAVA MAL (provado com os tres recibos enviados as 00:20 de 21/09):
--   1. NAO HAVIA LINHA DE CORRIDAS. O ganho das corridas TVDE ia somado dentro
--      da linha "Entregas". O Valdemir recebeu um recibo a dizer "Entregas x1 —
--      57,00 EUR" quando fez 1 entrega e 13 CORRIDAS. A Erika recebeu
--      "Entregas x0 — 4,00 EUR": zero entregas e ainda assim dinheiro.
--   2. AS LINHAS NUNCA SOMAVAM AO TOTAL. Faltava a parcela dos reembolsos das
--      compras adiantadas, e o "Acerto de cash devido" aparecia como se fosse
--      uma parcela quando ja esta contido nas outras.
--   3. O DINHEIRO EM MAO APARECIA A SOMAR. "Cash recebido de clientes +18,32"
--      quando esse dinheiro e para DEVOLVER a Bora — tem de aparecer a subtrair,
--      como o Uber e o Glovo mostram.
--
-- REGRA QUE FICA: as linhas do recibo somam, ao centimo, o saldo da semana.
-- Nenhuma parcela fica de fora e nenhuma aparece com o sinal trocado. Linha a
-- zero nao se mostra, para o recibo nao ter ruido.
--
-- Os blocos das limpezas, servicos, parceiros e lavagens ficam como estavam.
...
BEGIN
  ...
  SELECT s.week_start_at, s.week_end_at, 'driver', s.driver_id::text,
      ...
      || (CASE WHEN round(COALESCE(s.total_cash_received,0) * 100)::int <> 0
               THEN jsonb_build_array(jsonb_build_object(
                      'label','Dinheiro que recebeu em mao (devolve a Bora)',
