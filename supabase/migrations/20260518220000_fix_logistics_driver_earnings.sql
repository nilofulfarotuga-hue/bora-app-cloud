-- Fix logistics driver earnings in pricing_calculate.
--
-- BUG: v_is_package branch was using v_distance (total km) for the per-km
-- rate instead of v_extra_dist (km beyond base distance), and was incorrectly
-- adding c_shopping_driver_bonus which is not part of the logistics formula.
--
-- CORRECT formula (business_rules §logistics):
--   driver_earnings = logistics_driver_base + max(0, km - base_distance) * logistics_driver_per_km
--
-- Example 6 km: €4.00 + (6-4) × €0.50 = €5.00  (was: €4.00 + 6×€0.50 + €0.80 = €7.80)
...
BEGIN
  ...
  ELSIF v_is_package THEN
    -- FIX 2026-05-18: per-km applies only to km BEYOND base distance (v_extra_dist),
    -- not to total distance. c_shopping_driver_bonus is NOT part of the logistics formula.
    v_extra_dist          := GREATEST(0, v_distance - c_package_base_distance_km);
    ...
    v_driver_earnings     := ROUND(c_logistics_driver_base_pay + (c_logistics_driver_per_km * v_extra_dist) + v_apt_driver, 2);
...
COMMENT ON FUNCTION public.pricing_calculate(text, numeric, numeric, boolean, boolean, boolean) IS
  'Fix 2026-05-18: logistics driver_earnings now uses v_extra_dist (km beyond base) not v_distance, and removes incorrect c_shopping_driver_bonus. Customer delivery_fee unchanged.';
