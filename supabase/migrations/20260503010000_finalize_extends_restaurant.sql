-- BUG 4: extend finalize_storeshopping_purchase to accept service_type
-- 'restaurant' (in addition to existing 'storeShopping').
--
-- Bag fee rules:
--   restaurant + is_partner_store=true  → 0€ (partner absorbs)
--   restaurant + is_partner_store=false → 0.30€ fixed (non-partner restaurant)
--   storeShopping                       → unchanged (per-bag legacy logic)
--
-- For restaurant orders the driver does NOT substitute items: defaults
-- v_canon_status to 'bought' (vs 'pending' for storeShopping) so the
-- subtotal recalc lands on the original checkout subtotal automatically.

CREATE OR REPLACE FUNCTION public.finalize_storeshopping_purchase(
  p_order_id text,
  p_items_status jsonb,
  p_items_added jsonb DEFAULT '[]'::jsonb,
  p_bag_count integer DEFAULT NULL::integer
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Bag fee logic: restaurant uses fixed value (BR §bags); storeShopping
  -- keeps per-bag legacy logic (Sessão 3 will revise that path).
  IF v_is_restaurant THEN
    IF COALESCE(v_order.is_partner_store, false) THEN
      v_bag_fee_cents := 0;
    ELSE
      v_bag_fee_cents := 30; -- BRBags.RESTAURANT_BAG_FEE = 0.30
    END IF;
  ELSE
