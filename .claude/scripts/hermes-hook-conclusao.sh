...
#     · continuacao >= MAX_CONT -> genuinamente preso -> Telegram (decisão: reformular/arquivar).
#   ZONA_VERMELHA:
#     · dinheiro / destrutivo / ambíguo -> Telegram + espera o Danilo. NUNCA auto.
#
# TELEGRAM ao Danilo SÓ em 2 casos: (a) missão inteira terminou; (b) precisa de decisão dele.
#
  ...
  case "$estado" in
    aprovada)
      if [ -n "$missao" ]; then
        ...
        if [ -n "$prox" ]; then
          ...
          notify "$(resumo_missao "$missao")"
          log "MISSÃO $missao: CONCLUÍDA (última parte $parte) -> Telegram resumo."
        fi
      else
    ...
    travada)
      ...
      if [ "$cont" -lt "$teto" ] 2>/dev/null; then
        ...
        log "ordem $id: $tipo + continuações esgotadas ($teto) -> ARQUIVO (daily-pulse). SEM Telegram (I4)."
      fi
      ;;
    zona_vermelha)
...
selftest(){
  local tmp; tmp=$(mktemp -d); FILA="$tmp"; LOG="$tmp/hook.log"; local ok=0 fail=0
  ...
  echo "== T2: última parte aprovada -> sem promoção (missão fecha, notify) =="
  printf 'id: o3\nestado: aprovada\nmissao: mfim\nparte: 2\ntarefa: c\n' > "$tmp/o3.md"
  out=$(SELFTEST=1 decide "$tmp/o3.md" aprovada 2>&1); # notify imprime [NOTIFY]
  ...
  echo "== T4: travada continuacao>=MAX -> escala (nada de nova ordem) =="
  printf 'id: o5\nestado: travada\ntarefa: xyz\ncontinuacao: 2\n' > "$tmp/o5.md"
  before=$(ls "$tmp"/ordem-*.md 2>/dev/null | wc -l)
  decide "$tmp/o5.md" travada
  after=$(ls "$tmp"/ordem-*.md 2>/dev/null | wc -l)
