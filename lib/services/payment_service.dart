...
class PaymentService {
  ...
  ///
  /// Throws [StateError] if backend URL is empty.
  /// Throws [Exception] on HTTP / backend errors.
  Future<Map<String, dynamic>> createPaymentIntent({
    ...
    _assertOk(response, 'create-payment-intent');

    final body = jsonDecode(response.body) as Map<String, dynamic>;
    if (body['clientSecret'] == null || body['paymentIntentId'] == null) {
      throw Exception(
          '[PaymentService] create-payment-intent: incomplete response: $body');
    }
    debugPrint(
        '[PaymentService] intent created: ${body['paymentIntentId']}');
    return body;
  }
  ...
  /// Full card checkout: create intent → present sheet → return success/failure.
  /// For use in screens that do not go through [OrderStore.createOrder].
