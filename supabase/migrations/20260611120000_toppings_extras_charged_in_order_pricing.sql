-- T1 (2026-06-11): toppings/extras (product_option_items.price_add) não eram
-- cobrados — create_order/quote_order_pricing somavam só products.price × qty.
-- Fix: helper order_line_options_extras resolve as opções escolhidas (vindas em
-- p_input->'items'[].selected_options como {group, items:[nomes]}) por match
-- (product_id, nome do grupo, nome do item, is_available) e devolve o total de
-- price_add + a versão "priced" para o histórico. As 2 RPCs somam os extras ao
-- subtotal ANTES do ×1.15 não-parceiro: (base + extras) × qty → ×1.15.
-- create_order grava items enriquecidos com selected_options_priced (nome+preço)
-- para cliente/parceiro/estafeta verem o que foi pedido e cobrado.
-- Compat: payloads sem selected_options (builds <280) → extras 0, sem mudança.
-- Aplicada em prod via MCP apply_migration ("toppings_extras_charged_in_order_pricing").

CREATE OR REPLACE FUNCTION public.order_line_options_extras(
  p_product_id TEXT,
  p_selected_options JSONB
)
RETURNS TABLE(extras_total NUMERIC, options_priced JSONB)
LANGUAGE sql
STABLE
...
AS $fn$
  WITH sel AS (
    SELECT g.value->>'group' AS group_name,
           it.item_name
    FROM jsonb_array_elements(COALESCE(p_selected_options, '[]'::jsonb)) g
    CROSS JOIN LATERAL jsonb_array_elements_text(COALESCE(g.value->'items', '[]'::jsonb)) it(item_name)
  ),
  matched AS (
    -- MIN + FILTER: nunca cobra a mais (nome duplicado no grupo → o mais barato;
    -- item indisponível ou sem match → 0).
    SELECT s.group_name, s.item_name,
           COALESCE(MIN(poi.price_add) FILTER (WHERE poi.is_available), 0) AS price_add
    FROM sel s
    LEFT JOIN product_option_groups pog
      ON pog.product_id = p_product_id
     AND pog.name = s.group_name
    LEFT JOIN product_option_items poi
      ON poi.group_id = pog.id
     AND poi.name = s.item_name
    GROUP BY s.group_name, s.item_name
  ),
  grouped AS (
    SELECT group_name,
           jsonb_agg(jsonb_build_object('name', item_name, 'price_add', price_add)) AS items_priced,
           SUM(price_add) AS group_total
    FROM matched
    GROUP BY group_name
  )
  SELECT COALESCE((SELECT SUM(group_total) FROM grouped), 0)::NUMERIC,
         COALESCE((SELECT jsonb_agg(jsonb_build_object('group', group_name, 'items', items_priced)) FROM grouped), '[]'::jsonb);
$fn$;

REVOKE ALL ON FUNCTION public.order_line_options_extras(TEXT, JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.order_line_options_extras(TEXT, JSONB) FROM anon;
REVOKE ALL ON FUNCTION public.order_line_options_extras(TEXT, JSONB) FROM authenticated;
...
BEGIN
  ...
  THEN
    ...
    -- Lidas de p_input->'items' (CartItem.toJson: productId + selected_options
    -- com NOMES por grupo). Somadas ANTES do ×1.15 não-parceiro:
    -- (base + extras) × qty → ×1.15. Items ficam enriquecidos com
    -- selected_options_priced ({group, items:[{name, price_add}]}) para o
    -- histórico mostrar o que foi cobrado. Payload sem options → inalterado.
...
BEGIN
  ...
  THEN
    ...
    -- T1 (2026-06-11): somar price_add das opções escolhidas (mesma lógica do
    -- create_order — exibido tem de ser igual a cobrado ao cêntimo).
    v_items_in := COALESCE(p_input->'items', '[]'::jsonb);
    IF jsonb_typeof(v_items_in) = 'array' AND jsonb_array_length(v_items_in) > 0 THEN
      FOR v_item IN SELECT * FROM jsonb_array_elements(v_items_in)
      LOOP
