// PROPOSTA — NAO PUBLICADA (revertida a 23/09: reembolso automatico recusado com 403).
// =============================================================================
...
//   auto_refund_roundtrip_reservation -> accao NOVA (service_role): devolve o
//     PACOTE (paid_cents do vale). A auto_refund_reservation de sempre fica
//     intacta, byte a byte.
//
...
// So seguro porque a funcao corre com verify_jwt=true: a porta da Supabase
// ja validou a assinatura do JWT antes de o pedido chegar aqui.
function jwtEhServiceRole(token: string): boolean {
  try {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'auto_refund_roundtrip_reservation') {
      ...
      // papel do JWT.
      if (!(SERVICE_KEY && token === SERVICE_KEY) && !jwtEhServiceRole(token)) {
        return json({ error: 'not_service_role' }, 403);
      }
      const rideId = String(body.ride_id ?? '');
      if (!rideId) return json({ error: 'missing_ride_id' }, 400);
