...
data: 2026-07-13
corrida: 12a (gatilho FALLBACK_30MIN)
estado: atual
---

# Aprovador-vermelho — 12ª corrida (2026-07-13, FALLBACK 30MIN)

## Contexto do gatilho
Fallback de 30 minutos: item `nova` mais antigo parado ≥30 min sem triagem normal (watermark).
Ordem: re-triar TODA a fila `status='nova'` do zero, pela prova, não pelo histórico das 11
...
`ojykpzwqrtusfeakzrna`) — **exatamente o mesmo lote de 5 itens** das 11 corridas anteriores.
...
- `268aad47` — evidência é `pg_stat_statements` puro (420 chamadas, 106,7ms médios) — leitura de
  performance, **mas** a proposta é "otimizar `bora_dispatch_maintenance()`", função que faz
  `UPDATE orders` (cancela pagamentos abandonados) + chama `net.http_post` para a Edge Function
  `dispatch-engine`. Sem prova positiva de "sem escrita" na mudança proposta → **Balde B**.
- `abeca5d7` — mesma forma (evidência = performance), mas a função grava
  `deposit_status='retained'` (retém dinheiro do cliente em no-show de agendamento). **Balde B**.
- `85d8911b` — proposta é literalmente nova lógica de reatribuição/matching automático (escrita
  em `orders`/atribuição de motorista). Núcleo de dispatch. **Balde B**.
...
**0 Balde A, 0 auto-aprovados** nesta corrida. Classificação das 11 corridas anteriores
**confirmada, não corrigida** — reavaliei a evidência bruta de cada item e cheguei à mesma
conclusão de forma independente.

## Ação tomada
- Nenhuma mudança de roteamento (fila idêntica às corridas 3a-11a, decisão idêntica).
- Nenhuma lógica de dinheiro/dispatch/RLS tocada — só leitura (`execute_sql` SELECT) + 1 INSERT
  de auditoria (rasto).
- Telegram: **não enviado** — mesmo lote já surfaçado 11x hoje ao Danilo; reenviar seria spam.
- Reconfirmação registada em `admin_audit_log` (id `9a0cc866-36f2-4574-8317-6965572ae133`,
  action=`robot_suggestion_baldeB_reconfirmado`, entity_type=`robot_suggestion`, 5 ids +
  corrida="6a reconfirmacao do dia" + trigger=FALLBACK_30MIN).
...
## Anomalia (já reportada nas corridas 8a-11a — agora 5 registos consecutivos)
`FALLBACK_30MIN` continua a disparar em cadência curta sobre a mesma fila inalterada (12
...
desperdiçada e ruído crescente em `admin_audit_log`/inbox. Correção sugerida (fora do mandato
de roteamento deste agente): cooldown por-lote no `hermes-aprovador-vermelho.sh` (não re-disparar
se o conjunto de ids Balde B pendentes for idêntico ao da última corrida há <2h, ou backoff
exponencial). Encaminhar a `maestro-autonomia` ou ao Danilo — 5ª vez a sinalizar, recomenda-se
ação agora em vez de mais um adiamento.

## HANDOFF → bibliotecario-cerebro
tipo: facto
escopo: agente:aprovador-vermelho
tema-alvo: `permanente/procedural/aprovador-vermelho-triagem.md` (tabela "Histórico de corridas")
conteudo: 2026-07-13, corrida 12a (FALLBACK 30MIN), fila `nova` idêntica às corridas anteriores
...
agora com 5 registos consecutivos (8a-12a) sem correção — cooldown/backoff no script segue
pendente, encaminhado a `maestro-autonomia`/Danilo (recomenda-se agir, não só registar de novo).

## Ponteiros
`zonas-protegidas.md`, `business-rules.md`, `aprovador-vermelho-triagem.md`,
`project_aprovador_vermelho_central.md` (memória),
`.claude/.ai/knowledge/inbox/aprovador-vermelho-2026-07-13-11a-corrida.md`.
