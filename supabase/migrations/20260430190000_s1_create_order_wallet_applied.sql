-- S1: Mixed payment (wallet + Stripe/MBWay/cash). Adds wallet_applied_cents
-- to orders + makes create_order debit wallet atomically within the same
-- transaction. Reuses wallet_debit_for_order RPC (already SECURITY DEFINER).
--
-- Semantics:
--   orders.price = gross customer total (UNCHANGED — what cliente sees on receipt)
--   orders.wallet_applied_cents = how many cents came from saldo Bora
--   Stripe/MBWay charge = price*100 - wallet_applied_cents (computed by callers)

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS wallet_applied_cents INTEGER NOT NULL DEFAULT 0
...
BEGIN
  ...
  INSERT INTO orders (
    ...
    items, requires_car, order_type, wallet_applied_cents
  ) VALUES (
