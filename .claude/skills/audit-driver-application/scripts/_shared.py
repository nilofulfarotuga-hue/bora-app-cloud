...
def admin_identity():
    """(admin_user_id, admin_email) a partir do ambiente, p/ auditoria."""
    return os.environ.get("BORA_ADMIN_USER_ID", ""), os.environ.get("BORA_ADMIN_EMAIL", "")


def get_admin_jwt(ctx):
    """JWT de um utilizador admin (app_metadata.role='admin') p/ Edge Fns admin.

    Preferir BORA_ADMIN_JWT; senão sign-in com BORA_ADMIN_EMAIL+BORA_ADMIN_PASSWORD.
    A Edge Fn admin-force-driver-logout EXIGE este gate (não aceita service_role).
    """
    import requests
    ...
    if not (email and pw):
        raise RuntimeError(
            "Define BORA_ADMIN_JWT, ou BORA_ADMIN_EMAIL + BORA_ADMIN_PASSWORD, "
            "p/ obter um JWT de admin.")
...
def audit_log(ctx, action, entity_type, entity_id, details, *, text_id=False):
    """INSERT em admin_audit_log via service_role. text_id=True p/ ids TEXT (restaurants)."""
