-- ============================================================================
-- BORA — RPC finalize_storeshopping_purchase: aceitar p_bag_count
-- ============================================================================
-- Driver indica quantas sacolas usou no momento da finalização.
-- RPC calcula bag_fee server-side a partir de
-- platform_settings.bag_fee_supermarket_per_bag_cents (10c/saco).
--
-- Motivação BUG 27: 1ª tela do driver tem contador de sacolas, mas a chamada
-- updateOrderItems falhava silenciosamente (UPDATE directo a bag_fee bate no
-- trigger enforce_financial_immutability). Refactor faz a 1ª tela chamar
-- directamente esta RPC com o p_bag_count → fica autoritária sobre bag_fee.
--
-- Backwards-compat: p_bag_count IS NULL → usa v_order.bag_fee como antes.
-- ============================================================================
...
BEGIN
  ...
  -- Bag count: caller-provided (NEW) OR existing v_order.bag_count fallback.
  -- Compute v_bag_fee_cents authoritatively from platform_settings.
  ...
  ELSE
    -- Backwards-compat: derive from existing v_order.bag_fee.
    v_bag_fee_cents := ROUND(COALESCE(v_order.bag_fee, 0) * 100)::int;
  END IF;

  -- Items status payload → map id → status
  ...
  -- Persist (bag_count + bag_fee written here too when p_bag_count provided)
...
REVOKE ALL ON FUNCTION public.finalize_storeshopping_purchase(text, jsonb, jsonb, int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.finalize_storeshopping_purchase(text, jsonb, jsonb, int) TO authenticated;
