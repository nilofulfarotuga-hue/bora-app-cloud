-- ============================================================================
-- BORA — Sessão 3B-NOVA — create_order com gate + settlement (B5)
-- ============================================================================
-- Adiciona 2 blocos novos:
--  1. Gate: bloqueia se balance < wallet_max_negative_balance_cents
--  2. Settlement: liquida automaticamente saldo devedor anterior
--     (charge_total += |balance|, wallet → 0, log settlement)
--
-- Tudo dentro de SELECT FOR UPDATE para race-safety.
-- Kill-switch: platform_settings.wallet_negative_enabled = false → salta
-- ambos os blocos (comportamento Sessões 1+2+3 mantido bit-exact).
--
-- NÃO TOCAR no resto: pricing, menu_credit, INSERT, wallet_debit_for_order
-- ficam intactos. Apenas adiciona blocos no início e antes do INSERT.
-- ============================================================================
...
BEGIN
  ...
  -- ── Sessão 3B-NOVA: GATE WALLET NEGATIVO ─────────────────────────────────
  -- SELECT FOR UPDATE incondicional + bloqueio se < wallet_max_negative_balance_cents
  ...
  IF v_wallet_neg_enabled THEN
    ...
    IF v_wallet_balance_pre < v_wallet_max_neg THEN
      RAISE EXCEPTION 'WALLET_BLOCKED: saldo €%.2f, regularize antes de novos pedidos',
        v_wallet_balance_pre/100.0
        USING ERRCODE = 'P0001';
    END IF;
  END IF;
  -- ── Fim gate ─────────────────────────────────────────────────────────────
  ...
  IF v_wallet_cents > 0 THEN
    -- Já temos lock (FOR UPDATE acima); v_balance_check só para validação local.
    ...
    END IF;
    -- Não permite aplicar mais cents do que o saldo POSITIVO actual
    -- (parte negativa será liquidada via settlement abaixo).
  ...
  -- ── Sessão 3B-NOVA: SETTLEMENT DE SALDO DEVEDOR ANTERIOR ────────────────
  -- Se cliente entrou com saldo negativo, adiciona ao charge_total e zera wallet.
