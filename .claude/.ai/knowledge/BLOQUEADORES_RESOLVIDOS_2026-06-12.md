...
### B5 — HOTFIX (mesma data): paginação lazy partiu os ecrãs de loja → full-load on-demand ✅
O B5 v1 (lazy 30/página) PARTIU os mercados (build 288): os 3 ecrãs agrupam categorias EM MEMÓRIA, e com 30 produtos (todos de 1 categoria por `sort_order`) só aparecia 1 categoria + tela de categoria em branco. **Fix (Option B, cirúrgico):** `loadFullStoreProducts(id)` carrega a loja INTEIRA ao abrir (1000/página, notifyListeners progressivo), idempotente + **eviction** (só 1 mercado em memória; parceiros/restaurantes nunca em `_storeFullyLoaded`). Removido o andaime lazy (`ensureStoreProductsLoaded`/`loadMoreStoreProducts`/`_storeHasMore`/`storePageSize`). Ecrãs `market_store_tab`/`market_categories_tab`/`store_products_screen` voltam ao modelo em-memória que funcionava (0 mudanças de lógica; só initState→full-load + spinner). Ganho do arranque mantido (nunca 45k juntos; máx 1 loja ~12k Continente). Divergência do prompt (que pedia Option A=RPC+reescrever 3 ecrãs) justificada: hotfix de produção, DNA "funcionar sempre>features". Detalhe: `relatorios/HOTFIX_B5_PAGINACAO_LOJAS_2026-06-12.md`. Follow-up perf: Option A (RPC categorias) para first-paint instantâneo.

### B5 — Paginação lazy do catálogo (45k produtos) ✅ (v1 — substituída pelo hotfix acima)
