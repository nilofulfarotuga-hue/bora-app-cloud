...
2. **Doc drift fix (4 ficheiros, 9 ocorrências)**
   - `PROJECT_CONTEXT.md` — 4 refs "cash cap €30" → "€40" + cross-ref
     `platform_settings.max_cash_amount_cents`.
...
hardcoded by design para SLA, batching, tokens, fees fixas).
**Settings prod ausentes em TS:** 3 (`bag_fee_supermarket_per_bag_cents`,
`max_extra_charge_pct`, `wallet_negative_enabled`) — fora de scope
(TS não precisa, só Edge Functions específicas).

---

## Validação

- ✅ `flutter analyze`: 55 issues (baseline preservada — TS não afecta).
- ✅ Edge Functions consumers verificados (Q3): 3 activos
  (`client-cancel-order`, `execute-cancellation`,
  `cancel-order-with-choice`) + 1 dormente (`stripe-webhook` com
  `void` suppress).
- ✅ Comment em `stripe-webhook:197` já dizia "1.50 EUR retained" —
  confirma que €1.50 era a intenção desde sempre, código TS estava
  desalinhado.
