"""Orquestrador do onboarding de FARMÁCIA. DRY-RUN default; --commit aplica.

Como store, mas register-partner com category='pharmacy' e EXCLUI produtos sujeitos
a receita (MSRM). Reutiliza _partner_jwt do restaurant.

Uso: python insert_supabase.py --dir ".claude/.ai/onboard/wells-guarda" [--commit]
"""
...
def _build_register_body(r: dict) -> dict:
    return {
        ...
        "phone": str(r.get("phone", "")), "cuisineType": "", "category": "pharmacy",
...
def _read_products(onboard_dir: Path):
    """Devolve (otc_rows_src, blocked_rx) lendo o CSV e filtrando MSRM."""
    otc, blocked = [], []
    with (onboard_dir / "produtos.csv").open(encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            if not name:
                continue
            if validate_info.is_prescription(name):
                blocked.append(name)
                continue
            otc.append(row)
    return otc, blocked
...
def _dry_run_report(onboard_dir: Path, r: dict, state: dict, blocked: list[str]) -> None:
    cats, descs = state.get("categories", {}), state.get("descriptions", {})
    lines = [f"# Preview onboarding FARMÁCIA — {r['name']}", "",
             f"- categoria: pharmacy (is_partner=true) · licença INFARMED: {r.get('licenca_infarmed','-')}",
             f"- morada: {r['address']} · lat/lng: {r.get('lat')},{r.get('lng')}",
             f"- payload register-partner: `{_build_register_body(r)}`", ""]
    if blocked:
        lines += ["## ⚠️ BLOQUEADOS (receita — revisão manual)"] + [f"- {n}" for n in blocked] + [""]
    lines += ["## Produtos OTC a inserir"]
    for name in cats:
...
def main() -> int:
    ...
    otc_src, blocked = _read_products(onboard_dir)
    if blocked:
        log(f"⚠️ {len(blocked)} produto(s) MSRM bloqueados (não serão inseridos).", "WARN")

    if not args.commit:
        _dry_run_report(onboard_dir, r, state, blocked)
    ...
    try:
        ...
        log(f"register-partner OK → {restaurant_id} (pharmacy, pending)")
        ...
        for row in otc_src:
            ...
            rows.append({
                ...
                "is_available": True, "source": "onboard-partner-pharmacy",
            })
        if rows:
            pr = ctx.rest("POST", "products", jwt, rows)
            if pr.status_code >= 300:
                raise RuntimeError(f"INSERT produtos falhou: {pr.status_code} {pr.text[:200]}")
            log(f"{len(rows)} produto(s) OTC inseridos ({len(blocked)} MSRM excluídos).")

        state["committed"] = {"restaurant_id": restaurant_id, "products": len(rows), "blocked_rx": blocked}
        write_pipeline_json(onboard_dir, state)
        log(f"✅ Onboarding FARMÁCIA COMPLETO. restaurant_id={restaurant_id} (pending).")
        return 0
