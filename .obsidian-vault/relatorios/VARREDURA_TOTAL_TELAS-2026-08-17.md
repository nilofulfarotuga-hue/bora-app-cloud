...
**Números da noite**: 196 ecrãs inventariados · ~35k linhas lidas (F1-F4 integrais; F5/F6 dirigidas
por scanner) · **14 correções aplicadas em código** · 8 propostas (P1-P8) · 1 merge de rumo
(produção→varredura) · olho automático de 3 camadas montado e no CI · 0 erros no analyze em todos
os lotes · tudo pushed (CI de build NÃO disparado — só a autonomous-night-2026-04-29 publica).

**As 14 correções**: mini-mapa na oferta TVDE + cartão scrollable · estado de erro nos ganhos TVDE ·
ETA na estimativa do cliente TVDE · pesquisa na lista de restaurantes · B1 fallback legacy do menu ·
T1 botão com opções · taxa de entrega dinâmica no mercado (2 sítios) · avaliação real do estafeta
no tracking (era 4.9 fixo) · banner EN→PT · 3× "Teste"→"Mudar modo" · código de entrega por pedido
no stacking · priority_until tolerante a id/user_id · refetch de reservas+marcações no foreground ·
BoraAccentButton Flexible+ellipsis.

**Decisões de dinheiro à espera de "vai"** (nada aplicado): P1 fix da RPC driver_earnings_summary
(status+líquido) — URGENTE porque o ecrã do estafeta já a usa · P6 PIN server-side (P0 conhecido) ·
P8 admin reatribuir pedido · stash "tvde-plan-payment v10 tokens PENDENTE-VAI" intacto.

## Digest Hermes (8 linhas)

1. Varredura total tela-a-tela FEITA: 196 ecrãs, 4 papéis, noite de 16→17/08.
2. 14 correções de UI/estado aplicadas e pushed; analyze 0 erros; produção fundida na varredura.
3. TVDE motorista e cliente: nível Uber confirmado; oferta ganhou mini-mapa; estimativa ganhou ETA.
4. Dinheiro exibido: 4 furos "exibido≠cobrado/inventado" corrigidos (menu, opções, mercado, 4.9 fixo).
5. Bug pago-sem-receber corrigido: prioridade do estafeta falhava em conta id≠user_id.
6. 🔴 P1 urgente p/ Claude.ai: RPC driver_earnings_summary soma TVDE=0 (status errado) — SQL pronto.
7. OLHO AUTOMÁTICO ligado: fotos em memória a cada push + juiz de visão + Robo Test Lab (2 chaves do Danilo).
8. Gaps admin p/ autoridade total: reatribuir pedido (P8) e motivo de rejeição; relatório completo no repo.
